...
export const warnCommand: Command = {
  ...
  usage: "<@user | userID> <reason> [| additional info]",
  ...
  async execute(message: Message, args: string[]) {
    ...
    const rawText = getArgs(message, args).join(" ");
    ...
    const totalWarns = getInfractions(message.guild.id, target.id).filter(
      (i) => i.type === "Warn"
    ).length;
    ...
    await message.channel.send(`Warned ${target}.\n(${infraction.id})`);
    await message.delete().catch(() => {});

    await sendModLog(message.client, message.guild.id, {
      action: `Member Warned (Warn #${totalWarns})`,
    ...
    logger.info({ targetId: target.id, reason, caseId: infraction.id, totalWarns }, "User warned");
    ...
    const settings = await dbGet<any>("settings", message.guild.id).catch(() => null);
    const escalationSteps: Array<{ strikes: number; action: string; duration?: string }> =
      settings?.warnEscalation?.steps ?? [];
    const matchStep = escalationSteps.find((s) => s.strikes === totalWarns);

    if (matchStep && matchStep.action !== "warn") {
      const escReason = `Auto-escalation: reached ${totalWarns} warning${totalWarns === 1 ? "" : "s"}`;
      try {
        if (matchStep.action === "kick") {
          await (member as GuildMember).kick(escReason);
        } else if (matchStep.action === "ban") {
          await (member as GuildMember).ban({ reason: escReason, deleteMessageSeconds: 0 });
        ...
        await sendModLog(message.client, message.guild.id, {
          action: `⚠️ Escalation: ${matchStep.action.charAt(0).toUpperCase()}${matchStep.action.slice(1)} (${totalWarns} warning${totalWarns === 1 ? "" : "s"})`,
          executor: { tag: message.client.user!.tag, id: message.client.user!.id },
          target: { tag: target.tag, id: target.id },
          reason: escReason,
          color: 0xff6b6b,
        });
        logger.info({ targetId: target.id, action: matchStep.action, totalWarns }, "Warn escalation triggered");
      } catch (err) {
        logger.warn({ err, targetId: target.id }, "Warn escalation action failed");
      }
    }
  },
};
