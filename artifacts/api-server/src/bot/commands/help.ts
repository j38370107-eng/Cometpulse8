...
const CATEGORIES: { name: string; emoji: string; commands: string[] }[] = [
  ...
  },
  {
    ...
    emoji: "🎭",
    commands: ["modrole", "protectedrole", "addrole", "removerole"],
  },
  {
    name: "Configuration",
    emoji: "⚙️",
  ...
  },
  {
    name: "Tickets",
    emoji: "🎫",
    commands: ["ticket", "tblacklist", "tunblacklist"],
  },
  {
    name: "Applications",
    emoji: "📝",
    ...
    if (args[0]) {
      ...
      if (!cmd) {
        ...
        return message.reply(
          `❌ No command or shortcut named \`${args[0]}\` was found. Run \`${prefix}help\` to see all commands.`,
        );
      }
