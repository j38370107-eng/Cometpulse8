...
export const muteCommand: Command = {
  ...
  usage:
    "<@user | userID> [duration: 1s/5m/2h/1d] <reason> [| additional info]  — duration optional in role mode (omit for permanent)",
  ...
  async execute(message: Message, args: string[]) {
    ...
    const rest = getArgs(message, args);
    ...
    try {
      ...
      if (muteCfg.mode === "role") {
        ...
        await message.channel.send(`Muted ${target}${isPermanent ? " (permanent)" : ` (${duration})`}.\n(${infraction.id})`);
      ...
      await message.channel.send(`Muted ${target}.\n(${infraction.id})`);
      await message.delete().catch(() => {});
