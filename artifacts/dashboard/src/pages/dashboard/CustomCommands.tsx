...
export default function CustomCommands() {
  ...
  const save = async () => {
    const trigger = draft.trigger.trim().toLowerCase().replace(/\s+/g, "_");
    const response = draft.response.trim();
    if (!trigger) return show("Trigger word is required", "error");
    if (!response && !draft.embed.enabled) return show("Response is required (or enable embed)", "error");
    ...
    const dupe = commands.find(c => c.trigger === trigger && c.id !== modal.editing?.id);
    if (dupe) return show("A command with that trigger already exists", "error");
      ...
      <Modal open={modal.open} onClose={() => setModal({ open: false })} title={modal.editing ? "Edit Command" : "New Custom Command"} width={660}>
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", borderTop: "1px solid var(--border)", paddingTop: 14 }}>
            ...
            <Button onClick={save}>{modal.editing ? "Update" : "Create"}</Button>
          </div>
        </div>
      </Modal>
    </div>
