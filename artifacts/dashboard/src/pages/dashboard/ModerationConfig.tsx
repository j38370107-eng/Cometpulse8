...
export default function ModerationConfig() {
  ...
  const saved = useRef({
    ...
    logChannelId: "", warnExpiryDays: "30", warnEscalation: { steps: [] as any[] }, muteConfig: defaultMute,
  });
  ...
  const addStep = () => {
    ...
    steps.push({ strikes: lastStrikes + 1, action: "warn" });
  ...
  const save = async () => {
    ...
    try {
      ...
      saved.current = JSON.parse(JSON.stringify({ modRoles, protectedRoles, lockdownChannels, logChannelId, warnExpiryDays: String(warnDays), warnEscalation, muteConfig }));
    ...
    <div style={{ padding: "32px 32px 96px", maxWidth: 800 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <Card>
          ...
          <p style={{ fontSize: 12, color: "var(--text-secondary)", marginBottom: 16 }}>
            How long manually-issued warnings count toward escalation. Enter <strong>0</strong> for warnings that never expire.
          </p>
        ...
        <Card>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <Select label="Mute Mode" value={muteConfig.mode} onChange={(v) => setMuteConfig((c) => ({ ...c, mode: v as "timeout" | "role" }))}>
