...
import { Plus, Trash2, Edit2, Eye, ClipboardList, ArrowLeft, Link, ShieldOff } from "lucide-react";
...
export default function Applications() {
  ...
  const openEdit = (form: any) => {
    setFormDraft({ ...form, questions: (form.questions ?? []).map((q: any) => ({ choices: [], ...q })) });
  ...
  const addQuestion = () => {
    setFormDraft((f: any) => ({
      ...f,
      questions: [...f.questions, { id:generateId(), label:"", type:"short", required:true, choices:[] }]
    }));
  };
          ...
          <Card>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
              <div>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                  <input
                    ...
                    onChange={e => setAppCfg(c => ({ ...c, cooldownHours: Math.max(0, parseInt(e.target.value) || 0) }))}
              ...
              <Toggle
                ...
                checked={appCfg.notifyApplicant}
                onChange={v => setAppCfg(c => ({ ...c, notifyApplicant: v }))}
              />
            </div>
